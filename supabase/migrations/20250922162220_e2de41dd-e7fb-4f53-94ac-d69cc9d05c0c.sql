...
-- This prevents SQL injection and ensures functions use the correct schema

-- Fix deposit_credit function
CREATE OR REPLACE FUNCTION public.deposit_credit(user_id_param uuid, amount_param numeric, stripe_charge_id_param text DEFAULT NULL::text)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Create the game
  INSERT INTO games (creator_id, mode, stake, status, created_at)
  ...
  INSERT INTO ledger (user_id, game_id, amount, type, created_at)
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM game_players WHERE game_id = game_id_param AND player_id = auth.uid()) THEN
...
-- Fix update_elo_ratings function
...
BEGIN
  ...
  IF winner_id_param != game_record.creator_id AND 
     NOT EXISTS (SELECT 1 FROM game_players WHERE game_id = game_id_param AND player_id = winner_id_param) THEN
    RETURN JSON_BUILD_OBJECT('error', 'Winner is not part of this game');
  END IF;
  
  -- Calculate total pot and commission
  SELECT COUNT(*) + 1 INTO total_pot FROM game_players WHERE game_id = game_id_param;
  ...
  SELECT balance INTO winner_balance FROM profiles WHERE user_id = winner_id_param;
  ...
  INSERT INTO ledger (user_id, game_id, amount, type, platform_commission, created_at)
...
-- Fix create_withdrawal_request function
...
BEGIN
  ...
  IF stake_amount < 1.00 THEN
    RETURN jsonb_build_object('valid', false, 'error', 'Minimum stake is $1.00');
  END IF;
