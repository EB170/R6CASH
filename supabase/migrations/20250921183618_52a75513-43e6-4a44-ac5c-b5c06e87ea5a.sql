-- PHASE 3: Security Hardening & PHASE 4: Enhanced Monitoring (Fixed)

-- 1. Create audit table for security monitoring (if not exists)
...
DROP POLICY IF EXISTS "No public access to audit logs" ON public.audit_log;
...
-- 4. Enhanced game security - add constraints (if not exists)
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.check_constraints 
    WHERE constraint_name = 'valid_stake' AND table_name = 'games'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.check_constraints 
    WHERE constraint_name = 'valid_mode' AND table_name = 'games'
  ) THEN
    ALTER TABLE public.games 
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.check_constraints 
    WHERE constraint_name = 'positive_balance' AND table_name = 'profiles'
  ) THEN
